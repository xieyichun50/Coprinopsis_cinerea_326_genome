##usage
#Rscript gene_model_stats.R -i *.gff3 -s 8
...
option_list <- list(
  make_option(c("-i","--input"), type="character", default=NULL,
              ...
              dest="input"),
  make_option(c("-s","--skip-line"), type="numeric", default=NULL,
              help="skip lines with # [default %default]",
              dest="skip"),
...
##manual add files
{
  #opt$input<-"/mnt/content_176/yichun/pangenome/ref_genome/Cicer_arietinum_kabuli_genomic.gff"
}

##read in gff3 file
gff3<-read.delim(opt$input, header = FALSE, skip = opt$skip)
